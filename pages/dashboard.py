...
st.markdown("""
    ...
    .stTabs [data-baseweb="tab"] { height: 50px; white-space: pre-wrap; background-color: transparent; border-radius: 0px; border: 1px solid transparent; gap: 1px; padding-top: 10px; padding-bottom: 10px; font-weight: 600; color: rgba(255, 255, 255, 0.4); text-transform: uppercase; letter-spacing: 2px; font-size: 10px; transition: all 0.3s ease; }
    .stTabs [aria-selected="true"] { background-color: rgba(225, 29, 72, 0.05) !important; color: #e11d48 !important; border: 1px solid rgba(225, 29, 72, 0.3) !important; border-bottom: 1px solid transparent !important; }
    ...
    /* Buttons */
    .stButton>button { width: 100%; background-color: transparent; border: 1px solid rgba(225, 29, 72, 0.4); color: #e11d48; text-transform: uppercase; letter-spacing: 3px; font-size: 10px; transition: all 0.3s ease; border-radius: 0px; }
...
if "inv_data" in st.session_state:
    ...
    with tab1:
        ...
        st.markdown("<p style='color: rgba(255,255,255,0.5); font-size: 12px; font-style: italic; margin-bottom: 20px;'>Data Brief: A real-time ledger of your physical assets. This matrix tracks exact DMS status, calculates daily floorplan interest bleed, and cross-references CRM data to estimate unit demand.</p>", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        # Updated Brief to reflect the logic change
        st.markdown("<p style='color: rgba(255,255,255,0.5); font-size: 12px; font-style: italic; margin-bottom: 20px;'>Data Brief: The algorithm subtracts your 30-day sales velocity from your current stock. <span style='color: #4ade80; font-weight: bold;'>Green</span> indicates an overstock (surplus). <span style='color: #ff4d4d; font-weight: bold;'>Red</span> indicates a shortage (lost sales opportunity).</p>", unsafe_allow_html=True)
        ...
        # 1. SOFT HIGHLIGHT & REVERSED LOGIC:
        # Negative numbers (Shortages) are now Soft Red. Positive numbers (Overflow) are Soft Green.
        def color_delta_dark(val):
            if isinstance(val, int):
                # Using a very light background fill (0.08 opacity) and clear colored text.
                if val < 0: return 'background-color: rgba(225, 29, 72, 0.08); color: #ff4d4d; font-weight: bold;' # Soft Red Shortage
                elif val > 0: return 'background-color: rgba(34, 197, 94, 0.08); color: #4ade80; font-weight: bold;' # Soft Green Overflow
            return 'color: rgba(255, 255, 255, 0.1);' # Faded gray for zero
    ...
    with tab3:
        ...
        st.markdown("<p style='color: rgba(255,255,255,0.5); font-size: 12px; font-style: italic; margin-bottom: 20px;'>Data Brief: A hierarchical view of your inventory depth. Read from the center outward: Model Family -> Trim Level -> Exterior Paint. Click any segment to isolate and drill down.</p>", unsafe_allow_html=True)
        
        # JANE & RYE PALETTE: A randomized sequence of brand-aligned shades.
        jr_palette = [
            "#e11d48", # J&R Red
            "#1F2937", # Dark Slate
            "#FFFFFF", # Pure White
        ...
        fig.update_traces(
            ...
            # Removed forced white text so Plotly auto-contrasts black text on white slices and white text on dark slices
